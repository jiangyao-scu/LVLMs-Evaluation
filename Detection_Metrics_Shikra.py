...
def get_predBBoxes_general(pred_path, typeCoordinates, classID, dataset, data_path):
    myBoundingBoxes = BoundingBoxes()
...
def evaluate(gt_path, pred_path, classID, visual=False, dataset=None, data_lib=None):
    bboxes = get_predBBoxes_general(pred_path, typeCoordinates=BBType.Detected, classID=classID, dataset=dataset, data_path=data_lib[dataset])
    ...
    save_path = 'visual_box_new_Shikra_adjusted/{}/'.format(dataset)
    ...
    if visual:
        if dataset=='VisA':
            ...
            for img_name in img_names:
                ...
                key = key.replace('\\', '/')
                # key_sp = key.split('\\')
                # key = key_sp[0] + '/test/' + key_sp[3] + '/' + key_sp[4]
...
if __name__=='__main__':
    ...
    pred_path = 'results/DET_results/Shikra/VisA.lst'
